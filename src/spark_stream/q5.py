...
# rename event.time to event_time, select relevant columns
df4 = df3.withColumn("event_time", df3["event.time"])\
...
df5 = df4.where(col("rsvp_id").isNotNull())

# write to kudu master
df5.writeStream.trigger(processingTime="60 seconds")\
    .format("kudu")\
    .option("kudu.master",
    ...
    .option("checkpointLocation", "/user/jiayiyang/rsvp/kudu_checkpoint/checkpoint")\
    .outputMode("append")\
    .start()\
    .awaitTermination()
